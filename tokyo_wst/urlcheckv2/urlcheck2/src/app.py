...
def valid_ip(ip):
    try:
        ...
        print('valid_ip no exception',result.is_global)
        return result.is_global
    except:
        print('exception in valid_ip')
        return False

def valid_fqdn(fqdn):
    print('inside valid_fdqn')
...
def get(url, recursive_count=0):
    ...
    if 'location' in r.headers:
        if recursive_count > 2:
            return 'reccursion depth excceed'
        url = r.headers.get('location')
        if valid_fqdn(urlparse(url).netloc) == False:
            return 'in get() fdqn fails'
...
def admin_status():
    if flask.request.remote_addr != '127.0.0.1':
        return 'remote_addr is not localhost'
    return app.flag
...
def check_status():
    ...
    if valid_fqdn(urlparse(url).netloc) == False:
        return 'in check-status fdqn check failed'
    return get(url)
